				   ...
				   struct mt792x_link_sta *mlink)
{
	...
	mlink->wcid.idx = idx;
	mlink->wcid.phy_idx = mconf->mt76.band_idx; //sean wang: why is changed ?
	...
	if (vif->txq) { //sean wang: need to changed into per-link txq
		mtxq = (struct mt76_txq *)vif->txq->drv_priv;
		mtxq->wcid = idx;
	}

	//sean wang: need to add dbdc cmd here
out:
	return ret;
}
				      ...
				      struct ieee80211_link_sta *link_sta)
{
	...
	mt7925_mcu_sta_update(dev, link_sta, vif, true,
