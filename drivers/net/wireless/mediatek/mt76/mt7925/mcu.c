...
{
	struct mt792x_bss_conf *mconf = mt792x_link_conf_to_mconf(link_conf); //sean wang: could i use the convert functon for the legacy one ?
	struct {
		struct {
			u8 bss_idx;
			u8 pad[3];
			 ...
			 struct ieee80211_link_sta *link_sta)
{
	...
	amsdu->amsdu_en = true;
	msta->deflink.wcid.amsdu = true; //sean wang question
...
{
	struct mt792x_vif *mvif = (struct mt792x_vif *)vif->drv_priv;
	struct cfg80211_chan_def *chandef = &mvif->bss_conf.mt76.ctx->def; //sean wang, should be changed into per-bss config ?
	...
	phy->phy_type = mt76_connac_get_phy_mode_v2(mvif->phy->mt76, vif, chandef->chan->band, link_sta->sta); //sean wang, should pass per-sta config ?
	phy->basic_rate = cpu_to_le16((u16)vif->bss_conf.basic_rates); //sean wang, should be changed into per-bss config ?
	if (link_sta->ht_cap.ht_supported) {
		   ...
		   struct mt76_sta_cmd_info *info)
{
	...
	if (info->link_sta && info->enable) {
		...
		mt7925_mcu_sta_rate_ctrl_tlv(skb, info->vif, info->link_sta);
			    ...
			    struct ieee80211_chanctx_conf *ctx)
{
	...
	/* sean wang: why is band changed in ref. pactch ?
	if (band == NL80211_BAND_2GHZ)
               band = 1;
			    ...
			    int enable)
{
	struct mt792x_vif *mvif = (struct mt792x_vif *)vif->drv_priv;
        struct mt792x_bss_conf *mconf = mt792x_vif_to_link(mvif,
							   link_conf->link_id); //sean wang: work for the legacy one ?
	struct mt792x_dev *dev = phy->dev;
	struct sk_buff *skb;
	...
	mt7925_mcu_bss_basic_tlv(skb, link_conf, link_sta, ctx, phy->mt76,
				 mvif->sta.deflink.wcid.idx, enable); //sean wang: need to check again mvif->sta.deflink.wcid.idx
